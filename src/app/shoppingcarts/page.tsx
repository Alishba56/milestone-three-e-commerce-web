...
import img from '../../images/Elegance.png';
import img1 from '../../images/Urban .png';
import img2 from '../../images/blacksungalss.png';

const Page = () => {
  const [cartItems, setCartItems] = useState([
    { id: 1, name: 'Velvet Sneaker', size: 'MD', price: 20.00, quantity: 3, image: img2 },
    { id: 2, name: 'Smart Watch Timex', size: 'SM', price: 60.00, quantity: 2, image: img },
    { id: 3, name: 'French Connection', size: 'LG', price: 40.00, quantity: 2, image: img1 },
    { id: 4, name: 'Smart Watch', size: 'LG', price: 60.00, quantity: 2, image: img2 },
  ]);
  ...
  const handleAddItem = (id: number): void => {
    setCartItems(cartItems.map((item: CartItem) => 
      item.id === id ? { ...item, quantity: item.quantity + 1, price: item.price + item.price / item.quantity } : item
    ));
  };

  const handleDecreaseItem = (id: number): void => {
    setCartItems(cartItems.map((item: CartItem) => 
      item.id === id && item.quantity > 1 ? { ...item, quantity: item.quantity - 1, price: item.price - item.price / item.quantity } : item
    ));
  };
              ...
              <div key={item.id} className="grid grid-cols-3 items-start gap-4">
                <div className="col-span-2 flex items-start gap-4">
                  ...
                  <div className="flex flex-col">
                    ...
                    <button type="button" onClick={() => handleRemoveItem(item.id)} className="mt-6 font-semibold text-red-500 text-xs flex items-center gap-1 shrink-0">
                    <MdOutlineDelete />
                      REMOVE
                    </button>
                  </div>
                ...
                <div className="ml-auto">
                  <h4 className="text-lg max-sm:text-base font-bold text-gray-800">${item.price.toFixed(2)}</h4>
<div className='mt-6 flex items-center px-3 py-1.5 border border-gray-300 text-gray-800 text-xs outline-none bg-transparent rounded-md'>
                  <button type="button" onClick={() => handleDecreaseItem(item.id)}>
